  ...
  return (
    <div className="min-h-screen">
      <div className="mx-auto max-w-6xl px-6 py-10 lg:px-8">
        {campaign && (
          <div className="mb-6 flex items-center justify-between gap-3 rounded-2xl border-2 border-violet-300 bg-violet-50 p-4">
            <div className="flex items-center gap-2 text-sm">
              <span className="text-xl">📋</span>
              <div>
                <p className="font-semibold text-violet-900">Vous prospectez pour « {campaign.name} »</p>
                {campaign.sector && campaign.location && (
                  <p className="text-xs text-violet-700">{campaign.sector} à {campaign.location}</p>
                )}
              </div>
            </div>
            <Link href={`/campaigns/${campaign.id}`} className="rounded-lg bg-violet-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-violet-700">
              Voir la campagne
            </Link>
          </div>
        )}
        ...
        <header className="mb-8">
          <div className="flex items-center gap-3 mb-6">
            <div className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-600/30">
              <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                <path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z" />
              </svg>
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-blue-600">Vibecoder Prospect</p>
